   ...
   while (1)
{
 ...
 {          
         if(stop == 1){ 
            ...
            if(flag == 0) //2번 버튼을 처음 눌렀을때만 실행된다!
            {
                OCR0 = 0; flag =1; led = 0x01; 
            }   
                 

            if (direction_flag == 0)  //플래그 확인하고 올릴지 말지 결정하기!
            {
                OCR0++; 
                delay_ms(10);               
            }   
            ...
            if (OCR0 == 255) //플래그를 세우는 장소 정하기. 255에 도달하면 1로 바꿔주고, 그다음부터는 쭉 감소한다.
             {
                 direction_flag = 1;
             } 
             
            if (OCR0 == 0)  //0에 도착하면 플래그를 0으로 바꿔주고 전등 옆칸으로 옮겨주기. 255까지 쭉 OCR을 올려준다.
            {
                direction_flag = 0; 
...
//   } 
   ...
   else if (button1_flag == 0)
   {  
      button3_flag = 1; 
   }   
   
   else if (button2_flag == 0)
   { 
      button3_flag = 1;      
   }     
